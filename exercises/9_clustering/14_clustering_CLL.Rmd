...
2c. Do a PCA (as in the previous exercise) and perform k-means clustering on the first 20 PCs. How much does the clustering result differ from the clustering on all genes? Make a PCA plot of the first two components colouring patients by your clusters.
```{r}
# to be filled
```
...
3d. Use the `cutree` function on the output of 3b to create a vector containing clusters labels for the patients for 2 clusters.
```{r}
# to be filled
```
